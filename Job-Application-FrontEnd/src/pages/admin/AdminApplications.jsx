...
export default function AdminApplications() {
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      if (filters.education) {
        filtered = filtered.filter(
          (app) => app.applicant?.education === filters.education,
        );
      }
      ...
      if (filters.location) {
        filtered = filtered.filter(
          (app) => app.applicant?.location === filters.location,
        );
      }
      ...
      if (filters.cgpaMin !== "" || filters.cgpaMax !== "") {
        filtered = filtered.filter((app) =>
          inApplicantScoreRange(
            getApplicantCGPA(app.applicant),
            filters.cgpaMin,
            filters.cgpaMax,
          ),
        );
  ...
  const handleBulkStatusUpdate = async (status) => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to update ${selectedApplications.length} application(s) to ${status}?`,
      )
    )
      return;
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto">
              <Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <CardContent className="p-6 space-y-6">
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium">
                              {selectedApplication.applicant?.account?.name}
                            </span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              CGPA:{" "}
                              {formatApplicantMetric(
                                getApplicantCGPA(selectedApplication.applicant),
                              )}{" "}
                              · Exit exam (100%):{" "}
                              {formatApplicantMetric(
                                getApplicantExitExam(
                  ...
                  {/* Portfolio Links */}
                  {selectedApplication.applicant?.portfolioLinks &&
                    selectedApplication.applicant.portfolioLinks.length > 0 && (
                      <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                        ...
                        <div className="space-y-2">
                          {selectedApplication.applicant.portfolioLinks.map(
                            (link, index) => (
                              <a
                                key={index}
                                href={link}
